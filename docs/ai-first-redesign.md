...
- **Every** human message is understood by one `decide()` model call that
  ...
  answers, start/restart/decline the survey).
...
- All regex parsers, trigger phrases and the step state machine were **deleted**
  ...
  deterministic paths left are language-free: dedupe and the media apology.
- Unexpected answers are handled conversationally (e.g. refusing to give a name
  ...
  sentence.
- Safety: a degraded model turn never mutates state; the chat cannot erase data
  at all (people are pointed to hola@barrakudesbegur.org; an admin deletes from
  the admin panel); actions are validated by code (valibot) and carry no target.
