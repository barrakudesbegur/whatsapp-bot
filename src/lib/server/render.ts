...
function renderInbound(body: InboundBody): Partial<RenderedMessage> {
	...
	if (body.type === 'interactive') {
		const title =
			body.interactive?.button_reply?.title ?? body.interactive?.list_reply?.title ?? '';
		return { kind: 'text', text: title, reply: title };
	}
	for (const media of ['image', 'audio', 'video', 'sticker', 'document'] as const) {
...
export function renderMessage(row: MessageRow): RenderedMessage {
	let parsed: Partial<RenderedMessage>;
	try {
		const body = JSON.parse(row.body_json) as InboundBody & OutboundBody;
		parsed = row.direction === 'in' ? renderInbound(body) : renderOutbound(body);
	} catch {
	...
	if (row.ai_meta_json) {
		try {
			...
			if (m.model) ai = { model: m.model, latencyMs: m.latencyMs, tokens: m.tokens };
		} catch {
