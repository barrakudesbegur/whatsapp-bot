...
export class D1Store implements Store {
	...
	async getPerson(id: number): Promise<PersonRow | null> {
		return await this.db.prepare(`SELECT * FROM people WHERE id = ?1`).bind(id).first<PersonRow>();
	}
	...
	async insertInboundMessage(input: InsertInboundInput): Promise<boolean> {
		const row = await this.db
         ...
         RETURNING id`
			...
			.bind(input.waMessageId, input.personId, input.msgType, input.bodyJson, input.createdAt)
			.first<{ id: number }>();
	...
	async listKbEntries(activeOnly = false): Promise<KbEntryRow[]> {
		const { results } = await this.db
			.prepare(`SELECT * FROM kb_entries ${activeOnly ? 'WHERE active = 1 ' : ''}ORDER BY slug`)
			.all<KbEntryRow>();
		return results ?? [];
	}
	...
	async upsertKbEntry(input: UpsertKbEntryInput): Promise<KbEntryRow> {
		const row = await this.db
         ...
         RETURNING *`
			...
			.bind(input.slug, input.title, input.contentMd, input.active ? 1 : 0, input.at)
			.first<KbEntryRow>();
	...
	async listMessagesForPerson(personId: number): Promise<MessageRow[]> {
		const { results } = await this.db
			.prepare(`SELECT * FROM messages WHERE person_id = ?1 ORDER BY created_at ASC, id ASC`)
			.bind(personId)
			.all<MessageRow>();
		return results ?? [];
	}
