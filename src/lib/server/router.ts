...
import type { FlowContext, FlowInput, FlowModule, FlowResult } from './flows/types.ts';
...
import { normalizeText, parseYesNo } from './normalize.ts';
...
export function gdprIntent(text: string): boolean {
	...
	if (!n) return false;
	return /esborr.*dad|elimin.*dad|suprim.*dad|dad.*esborr|dret a l oblit|\brgpd\b|\bgdpr\b/.test(n);
}
...
function buildContext(person: PersonRow, instance: FlowInstanceRow | null): FlowContext {
...
async function applyResult(
	...
	if (result.deferToAi) {
		...
		if (result.deferToAi.interpret && !interpretedMeta) {
			...
			if (interp.value !== null) {
				...
				return applyResult(person, instance, flow, ctx, input, result2, deps, interp.meta);
			}
		}
		...
		const sentAnswer = await sender.send(person, [{ kind: 'text', body: answer.text }], {
	...
	if (result.patch) {
		...
		const newStep = 'step' in result.patch ? (result.patch.step ?? null) : instance.step;
...
async function startGdprErase(person: PersonRow, deps: RouterDeps): Promise<SentMessage[]> {
	const { store, sender } = deps;
	const now = nowIso();
...
async function handleGdprConfirm(
	...
	if (input.kind === 'button') {
		decision = input.id === 'gdpr_yes' ? 'yes' : input.id === 'gdpr_no' ? 'no' : null;
	...
	if (decision === 'yes') {
		...
		if (!ok) return [];
		const sent = await sender.send(person, [{ kind: 'text', body: GDPR_DONE }], {
			flowInstanceId: instance.id
		});
	...
	// Unclear → re-ask.
	return sender.send(person, [{ kind: 'text', body: GDPR_CLARIFY }, GDPR_CONFIRM], {
		flowInstanceId: instance.id
	});
}
...
async function hasCompletedSurvey(store: Store, personId: number): Promise<boolean> {
	const latest = await store.getLatestFlowInstance(personId, CURS_SARDANES);
	return latest != null && (latest.status === 'completed' || latest.completed_at != null);
}
