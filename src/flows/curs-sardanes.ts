...
export function parseAvailability(text: string): AvailabilityId | null {
  ...
  if (/\bigual\b|qualsevol|tot\s+em\s+va|m\s*es\s+igual|tant\s+me\s+fa/.test(n))
    return "igual";
  return null;
}
...
function parseActionText(text: string): ActionId | null {
  ...
  if (/\bres\b|gracies|no\s+vull|deixa|passo|res\s+de\s+res/.test(n))
    return "res";
  return null;
}
...
function onAction(ctx: FlowContext, input: FlowInput): FlowResult {
  ...
  if (input.kind === "button") {
    choice = (ACTION_IDS as readonly string[]).includes(input.id)
      ? (input.id as ActionId)
      : null;
...
function onAvailability(ctx: FlowContext, input: FlowInput): FlowResult {
  ...
  const complete = (
    availability: AvailabilityId | "custom",
    raw?: string,
  ): FlowResult => ({
    ...
    patch: {
      ...
      data:
        availability === "custom"
          ? { availability, availability_raw: raw ?? "" }
          : { availability },
    },
  });
...
function onConfirmRestart(ctx: FlowContext, input: FlowInput): FlowResult {
  ...
  if (input.kind === "button")
    yes = input.id === "yes" ? true : input.id === "no" ? false : null;
  else if (input.kind === "text") {
    const n = normalizeText(input.text);
    if (
      /\bsi\b|\bsisi\b|\bvale\b|\bval\b|\bok\b|dacord|clar|esclar|canviar/.test(
        n,
      )
    )
      yes = true;
