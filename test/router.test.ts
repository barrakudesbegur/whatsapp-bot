...
import { validateOutMessage } from "../src/messages.ts";
import type { SentMessage } from "../src/wa/sender.ts";
import type { WebhookEnvelope } from "../src/wa/wire.ts";
...
describe("full survey via the simulator path", () => {
  it("trigger → name → grup → availability → close, with correct D1 state", async () => {
    ...
    const inst = await h.store.getLatestFlowInstance(
      person!.id,
      "curs-sardanes",
    );
  ...
  it("avisam path also completes", async () => {
    ...
    const k4 = await button(
      h,
      WA,
      "avisam",
      pickKind(k2, "buttons")!.waMessageId,
    );
    const k5 = await list(
      h,
      WA,
      "diumenges",
      pickKind(k4, "list")!.waMessageId,
    );
...
describe("status webhooks", () => {
  it("updates the matching outbound message row (and logs failures)", async () => {
    ...
    await runWebhook(
      h,
      rawStatus(outId, "failed", [{ code: 131047, title: "boom" }]),
    );
  ...
  it("ignores a status for an unknown message id", async () => {
    const h = newHarness();
    await expect(
      runWebhook(h, rawStatus("wamid.NOPE", "read")),
    ).resolves.toEqual([]);
  });
});
...
describe("returning completed person", () => {
  ...
  it("re-triggering offers to change answers", async () => {
    ...
    expect((prompt!.message as { body: string }).body).toContain(
      "Ja et tenia apuntat",
    );
  ...
  it("'Sí' restarts at the action question", async () => {
    ...
    await complete(h);
    const again = await text(h, WA, TRIGGER);
    const yes = await button(
      h,
      WA,
      "yes",
      pickKind(again, "buttons")!.waMessageId,
    );
...
describe("GDPR erase", () => {
  ...
  it("declining keeps the data", async () => {
    ...
    const no = await button(
      h,
      WA,
      "gdpr_no",
      pickKind(confirm, "buttons")!.waMessageId,
    );
