...
import {
  ...
  parseAvailability,
  AVAILABILITY_IDS,
} from "../src/flows/curs-sardanes.ts";
import type { FlowContext, FlowInput } from "../src/flows/types.ts";
import { validateOutMessage, type OutMessage } from "../src/messages.ts";
...
describe("start (K1)", () => {
  it("asks the name and activates on step 'name'", () => {
    ...
    expect((r.messages[0] as { body: string }).body).toContain(
