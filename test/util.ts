...
import { StubAiProvider } from "../src/ai/provider.ts";
import { makeDeps, handleWebhook, type RouterDeps } from "../src/router.ts";
import {
  buildSimulatedWebhook,
  type SimulateInput,
} from "../src/wa/simulate.ts";
import type { Env } from "../src/types.ts";
import type { OutMessage } from "../src/messages.ts";
...
export function allText(sent: SentMessage[]): string {
  return sent
    .map((s) => (s.message.kind === "text" ? s.message.body : s.message.body))
    .join("\n");
}
